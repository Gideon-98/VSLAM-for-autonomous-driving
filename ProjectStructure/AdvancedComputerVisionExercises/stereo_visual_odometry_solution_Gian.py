...
class VisualOdometry():
	...
	def get_pose(self, i):
		...
		img1_l, img2_l = self.images_l[i - 1:i + 1] #should we put [i:i+1] ??
		
		# Get teh tiled keypoints
		kp1_l = self.get_tiled_keypoints(img1_l, 10, 20)
		
		# Track the keypoints
		self.tp1_l, self.tp2_l = self.track_keypoints(img1_l, img2_l, kp1_l)
		
		# Calculate the disparities
		self.disparities.append(np.divide(self.disparity.compute(img2_l, self.images_r[i]).astype(np.float32), 16))
		
		# Calculate the right keypoints
		self.tp1_l, self.tp1_r, self.tp2_l, self.tp2_r = self.calculate_right_qs(self.tp1_l, self.tp2_l, self.disparities[i - 1], self.disparities[i])
		
		# Calculate the 3D points
		self.Q1, self.Q2 = self.calc_3d(self.tp1_l, self.tp1_r, self.tp2_l, self.tp2_r)
