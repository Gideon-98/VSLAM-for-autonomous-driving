...
class VisualOdometry():
    ...
    def track_keypoints(self, img1, img2, kp1, max_error=4):
        ...
        #print("trackpoints1= ", trackpoints1,"trackpoints2= ", trackpoints2)
        return trackpoints1, trackpoints2
    #Keypoints er 1-1 det vil sige trackpoints[1] svarer til trackpints2[1]. Det er i pixelkoordinater og ikke worldcoordinater.
    ...
    def visualize_track(self, index, len):

        # This loads images starting a index = index, to the lenght defined by len
        image_list = []
        for i in range(index, len):
            image_list.append(self.images_l[i])
            #print(i)



        # Get the tiled keypoint at index
        kpm1 = self.get_tiled_keypoints(image_list[index], 10, 20)
        tp_list=[]
        ...
        for i in range(index, len-1):
            tp0, tp1 = self.track_keypoints(image_list[i], image_list[i + 1], kpm1)
            tp_list.append(tp1)
        ...
        for i in range(len):
            drawpoint = [cv2.KeyPoint(x, y, 1) for x, y in tp_list[i]]
            drawpoint_list.append(drawpoint)
        ...
        for i in range(index,len):
            output_image = cv2.drawKeypoints(image_list[i], drawpoint_list[i], 0, (0, 255, 0),
