...
class ListBundler:
    ...
    def duplicate_and_sort(self, temp_list):
        ...
        for i , x in enumerate(tqdm(temp_list[cur_frame_idx:len(temp_list)-1], initial=cur_frame_idx, unit="sorting"),start=cur_frame_idx):

            if temp_list[i][0] == temp_list[i - 1][0]:  # Check if the trackpoint we are looking at is the first of it's related frame. Happens first when we go from
